...
int user_parser P1(char *, buff)
{
    ...
    char *user_verb = 0;
    int where;
#ifdef PACKAGE_NEWMOON
    extern int au_aliases PROT ((char *));
    char *bufstart;
#endif
...
#ifdef PACKAGE_NEWMOON
    /* Strip leading whitespace (why do I have to add this?)
     * These bitties by LordSutch@NewMoon
     */
    for (bufstart = buff; (bufstart < (buff + strlen (buff))) &&
         (*bufstart == ' '); bufstart++)
      ;

    if (bufstart > buff)
    {
        memmove (buff, bufstart, strlen (bufstart) + 1);
    }
	...
	*p = ' ';
	...
	if (s->flags & (V_NOSPACE | V_SHORT)) {
...
#else /* PACKAGE_NEWMOON */
      /* Hacked out the useless if(1)...else business
       * I know it's optimized out, but it's still stupid code ;)
       * LordSutch@NewMoon 08/18/95
       */
      lv = strlen (s->verb);
      pp = strchr (buff, ' ');
      if (pp)
      ...
      if (s->star == lv)
      {
          if (strncmp (buff, s->verb, lv))
              continue;
      }
      else if (s->star != -1)
      {
	  /* Rewritten by LordSutch@NewMoon, 950817 */
	  if (lv && ((lv != lb) || strncmp (buff, s->verb, lv)))
          {
	      if (lb < s->star || lb > lv)
		  continue;
	      if (strncmp (buff, s->verb, lb))
  		  continue;
          }
      }
      else
...
#endif
#ifdef PACKAGE_NEWMOON
      if (s->star == strlen (s->verb))
      {
	  if (!s->star)
	      push_constant_string (buff);
	  else if (*(buff + s->star) == '\0')
	      push_null ();
	  else
	      push_constant_string (buff + s->star);
      } else if (s->star != -1) {
	  char *pp;

	  pp = strchr (buff, ' ');
	  if (pp && (pp - buff) >= s->star && *(pp + 1))
	  {
	      push_constant_string (pp + 1);
	  }
	  else
	  {
	      push_null ();
...
#ifdef PACKAGE_NEWMOON
int star_compare P2 (int, star1, int, star2)
{
    if (star1 == -1)
        star1 = 127;
    if (star2 == -1)
...
void add_action P3(svalue_t *, str, char *, cmd, int, flag)
{
    ...
    if (cmd) {
      ...
      /* Work out where (if there is) the star is, and store
       * the priority
       */
      p->pri = flag;
      ii = 0;
      buf = pp = (char *) MALLOC (strlen (cmd) + 1);
      strcpy (buf, cmd);
      while (*pp && *pp != '*')
      {
	  ii++;
  ...
  /* Okay. Priorities.
   * Chunder down the chain till we go too far, back up one,
   * and stick it in there. Neat huh? Not really...
   *
   * Reworked a little to prioritize things with asterisks a little
   * better.
   * LordSutch@NewMoon 951001
   */
    {
      sentence_t **q;

      q = &(command_giver->sent);
      while (*q && (*q)->pri > flag)
        q = &((*q)->next);
      
      while (*q && (*q)->pri >= flag &&
             (((*q)->pri > flag) ||
              (((*q)->pri == flag) && star_compare ((*q)->star, p->star))))
        q = &((*q)->next);
